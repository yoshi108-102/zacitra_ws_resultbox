...
└── src/
    ...
    │   └── folder.py       # FolderRecord データクラス
    ├── handlers/
    │   ├── create_upload.py       # POST /uploads
    │   ├── complete_upload.py     # POST /documents/{id}/complete
    │   ├── list_documents.py      # GET  /documents
    │   ├── create_folder.py       # POST /folders
    │   ├── list_folders.py        # GET  /folders
    │   ├── move_document.py       # POST /documents/{id}/move
    ...
    └── services/
        ...
        ├── folder_repo.py    # フォルダ用 DynamoDB アクセス
...
| `POST` | `/folders` | `handlers.create_folder` | 単一階層のフォルダを作成 |
| `GET` | `/folders` | `handlers.list_folders` | 自分のフォルダ一覧を取得 |
| `POST` | `/documents/{document_id}/move` | `handlers.move_document` | PDF をルートまたは別フォルダへ移動 |
| `DELETE` | `/documents/{document_id}` | `handlers.delete_document` | PDF とメタデータを削除 |
...
| `folder_id` | str \| null | 所属フォルダ。未設定ならルート |

`to_public_dict()` で `owner_sub` と `s3_key` を除いた公開向けの辞書を返す。

### FolderRecord (`models/folder.py`)

| フィールド | 型 | 説明 |
|-----------|-----|------|
| `owner_sub` | str | Cognito ユーザー ID |
| `folder_id` | str | UUID hex |
| `folder_name` | str | 表示名 |
| `created_at` | str | ISO 8601 |
| `updated_at` | str | ISO 8601 |

フォルダは単一階層のみをサポートし、空フォルダも DynamoDB に保持する。
...
```
documents/{owner_sub}/root/{document_id}/{sanitized_file_name}.pdf
documents/{owner_sub}/folders/{folder_id}/{document_id}/{sanitized_file_name}.pdf
```
...
| `update_document_location(...)` | S3 キーと `folder_id` を更新 |
| `delete_document_record(owner_sub, document_id)` | DeleteItem |

### `services/folder_repo.py`

| 関数 | 操作 |
|------|------|
| `create_folder(record)` | PutItem |
| `get_folder(owner_sub, folder_id)` | GetItem |
| `list_folders(owner_sub)` | Query |
...
| `copy_document_object(source_key, destination_key)` | S3 上で PDF を移動するためのコピー |
| `delete_document_object(s3_key)` | S3 オブジェクト削除 |
...
| `invalid_folder` | 400 | フォルダ名不正 |
| `invalid_folder_id` | 400 | 存在しないフォルダを指定 |
| `document_not_ready` | 409 | ドキュメントが ready 状態でない |
| `folder_already_exists` | 409 | 同名フォルダが既に存在する |
| `upload_create_failed` | 500 | DynamoDB 書き込み失敗 |
| `complete_failed` | 500 | 完了時 DynamoDB 更新失敗 |
| `document_move_failed` | 500 | PDF の移動失敗 |
| `document_delete_failed` | 500 | PDF の削除失敗 |
